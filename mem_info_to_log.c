...
void write_Mem_Information() {
    ...
    ((fd = open(MEM_INFO_LOG, O_WRONLY | O_CREAT | O_APPEND, (S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP ) & (~S_IRWXO))) == -1) ? exception(-1, "write_Mem_Information", "Memory Information Log") : 0; 
    // Create or Open log file (640)
    content.date = get_Date();
    content.size = get_Mem_Usage();
    (write(fd, &content, sizeof(MEM_Info)) != sizeof(MEM_Info)) ? exception(-3, "write_Mem_Information", "Memory Information Log") : 0; // Save Memory Information (Free, Total Size)
    close(fd);
}
...
MEM_Size get_Mem_Usage() {
    ...
    ((fp = fopen("/proc/meminfo", "r")) == NULL) ? exception(-1, "get_Mem_Usage", "Memory Usage") : 0;
    for (; fscanf(fp, "%s %ld", buf, &size_buf) != EOF; allocation_Value_to_Var(&res, buf, size_buf));
    fclose(fp);    
    ((res.mem_free == -1) || (res.mem_total == -1) || (res.swap_free == -1) || (res.swap_total == -1)) ? exception(-2, "get_Mem_Usage", "Memory Size(Usage)") : 0;
    return res;
}
