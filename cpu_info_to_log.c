...
CPU_Usage get_CPU_Jiffies(){
    ...
    CPU_Usage usage;
    FILE *fp = NULL;
    ((fp = fopen("/proc/stat", "r")) == NULL) ? printf("%s\n", exception(-1, "get_CPU_Jiffies", "/proc/stat")) : 0; // /proc/stat File Open Error
...
float get_CPU_Temperature(){
    ...
    for (int i = 1; (fp = fopen(path_type, "r")) != NULL; i++){ // Find Temperature File Location
        (fscanf(fp, "%s", type) == EOF) ? printf("%s\n", exception(-2, "get_CPU_Temperature", "Device Type")) : 0;
    ...
    (fp == NULL) ? printf("%s\n", exception(-1, "get_CPU_Temperature", "Device Type")) : 0;  // Device Type File Open Error
    strncpy(path_temp, path_type, strlen(path_type) - 4); // get CPU Temperature File Location
    strcat(path_temp, "temp");
    fclose(fp);
    fp = NULL;
    ((fp = fopen(path_temp, "r")) == NULL) ? printf("%s\n", exception(-1, "get_CPU_Temperature", "CPU Temperature")) : 0; // CPU Temperature File Open Error
    (fscanf(fp, "%f", &temp) == EOF) ? printf(exception(-2, "get_CPU_Temperature", "CPU Temperature")) : 0; // CPU Temperature Data Read Error
    fclose(fp);
    return temp/1000;
}
